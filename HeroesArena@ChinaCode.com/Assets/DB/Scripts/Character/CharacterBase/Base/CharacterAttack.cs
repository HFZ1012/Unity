...
public class CharacterAttack : MonoBehaviour
{
	...
	public void DoDamage ()
	{
		...
		foreach (var hit in colliders) {
			...
			for(int i=0;i<character.TargetTag.Length;i++){
				if(hit.gameObject.tag == character.TargetTag[i]){
					targethit = true;	
				}
			}*/
			...
			DamageManager dm = hit.gameObject.GetComponent<DamageManager> ();
			...
			if (dm) {
				...
				int damage = character.characterStatus.Damage;
				AverageDamage = (int)Random.Range (damage / 2.0f, damage) + 1;

				
				dm.ApplayDamage (AverageDamage, dirforce, this.gameObject);
				// Add Particle Effect
				dm.AddParticle (hit.transform.position + Vector3.up);
			}
