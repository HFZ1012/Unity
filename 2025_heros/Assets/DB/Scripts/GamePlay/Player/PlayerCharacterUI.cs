...
public class PlayerCharacterUI : ItemUI
{
	...
	void Awake ()
	{
		...
		if((GetComponent<NetworkView>() && !GetComponent<NetworkView>().isMine) && (Network.isServer || Network.isClient)){
			Destroy(this);
		}
	}
	...
	void OnGUI ()
	{
		...
		if (character.characterStatus) {
			...
			itemRenderer.Draw (character.characterInventory.ItemSlots, character.characterInventory, ItemRenderMode.Player, "Inventory");
		...
		if (!Screen.lockCursor) {
			...
			if (GUI.Button (new Rect (20, 20, 50, 50), "I I")) {
				...
				if(gameManae){
					gameManae.IsPlaying = !gameManae.IsPlaying;
					gameManae.Page = 0;
				}
			}
		} else {
